...
class TestStackComponent:
    ...
    async def test_create_stack_from_blueprint_smart_elicitation_fallback(
        ...
        mock_result.stdout = "Stack created successfully"
        mock_result.stderr = ""
        mock_result.exit_code = 0
        mock_execute_cli_command.return_value = mock_result

        async with Client(stack_server) as client:
            # Test with missing parameters (no elicitation support)
...
"""
    ...
    async def test_create_stack_directly_invalid_use_case(
        ...
        handler = StackHandler(cli)

        # Test with invalid use case
        result = await handler.create_stack_directly(
            ref="cycloid-io:terraform-sample",
            name="test-stack",
            use_case="invalid-use-case",
            service_catalog_source_canonical="cycloid-stacks-test",
            available_use_cases=["aws", "gcp"],
        )
    ...
    async def test_create_stack_directly_catalog_repository_error(
        ...
        # Test with catalog repository error
        result = await handler.create_stack_directly(
            ref="cycloid-io:terraform-sample",
            name="test-stack",
            use_case="aws",
    ...
    async def test_create_stack_directly_invalid_canonical(
        ...
        # Test with invalid canonical
        result = await handler.create_stack_directly(
            ref="cycloid-io:terraform-sample",
            name="test-stack",
            use_case="aws",
            service_catalog_source_canonical="invalid-canonical",
            available_use_cases=["aws", "gcp"],
        )
    ...
    async def test_create_stack_directly_success(
        ...
        assert "✅ Stack 'test-stack' created successfully" in result
